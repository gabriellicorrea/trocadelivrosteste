...
class Layout {
  ...
  static Widget render(Widget content, [String title = 'Troca']) {
    return Scaffold(
      ...
      //     QuerySnapshot queryProd = await Firestore.instance
      //         .collection('produto')
      //         .limit(1)
      //         .getDocuments();
      ...
      //     Lista de sabores
      //     QuerySnapshot querySab =
      //         await Firestore.instance.collection('sabor_pizza').getDocuments();
      //     Iterable<DocumentSnapshot> sabores =
      //         querySab.documents.where((DocumentSnapshot test) {
      ...
      //     QuerySnapshot queryUsr =
      //         await Firestore.instance.collection('usuarios').getDocuments();
      //     Iterable<DocumentSnapshot> usrs =
      //         queryUsr.documents.where((DocumentSnapshot test) {
      ...
      //     int counter = 1;
      //     sabores.forEach((DocumentSnapshot sabDoc) =>
      //         listSabores['sabor_' + (counter++).toString()] =
      //             sabDoc.reference);

      //     data['sabores'] = listSabores;

      //     print(listSabores);
      //     DocumentReference addido =
      //         await Firestore.instance.collection('pedido').add(data);
      //     print(addido);
      //   },
      //   tooltip: 'Increment',
